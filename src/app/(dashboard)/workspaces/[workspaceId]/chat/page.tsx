...
export default function ChatPage() {
  ...
  const [pollingStatus, setPollingStatus] = useState<
    "idle" | "loading" | "newMessages" | "error"
  >("idle");
  ...
  useEffect(() => {
    ...
    messageProcessorRef.current = (newMessage) => {
      ...
      if (processedMessageIds.has(messageUniqueId)) {
        console.log(
          `⏭️ Bỏ qua tin nhắn đã xử lý qua realtime: ${newMessage.$id}`
        );
        return;
      }
      ...
      setMessages((prevMessages) => {
        ...
        if (prevMessages.some((msg) => msg.$id === newMessage.$id)) {
          console.log(
            `⏭️ Bỏ qua tin nhắn đã có trong danh sách: ${newMessage.$id}`
          );
          return prevMessages;
        }
        ...
        if (isTempMessage) {
          console.log(
            `⚠️ Phát hiện tin nhắn tạm thời, thay thế bằng tin nhắn thật: ${newMessage.$id}`
          );
      ...
      try {
        ...
        setMessages((prevMessages) => {
          ...
          if (newMessages.length === 0) {
            console.log(
              "ℹ️ Không phát hiện tin nhắn mới trong kết quả polling"
            );
            return prevMessages;
          }

          console.log(
            `📥 Tìm thấy ${newMessages.length} tin nhắn mới khi polling`
          );
          ...
          newMessages.forEach((newMsg) => {
            ...
            if (tempIndex !== -1) {
              ...
              console.log(
                `🔄 Thay thế tin nhắn tạm ${mergedMessages[tempIndex].$id} bằng tin nhắn thật ${newMsg.$id}`
              );
        ...
        if (hasNewMessages) {
          ...
          const newMessagesFromOthers = sortedMessages.some(
            (msg) =>
    ...
    const fetchMemberId = async () => {
      try {
        const response = await fetch(
          `/api/members/me?workspaceId=${workspaceId}`
        );

        if (response.status === 404) {
          // User is not a member of this workspace
          setError(
            "You are not a member of this workspace. Please join the workspace to access chat."
          );
          setIsLoading(false);
          return;
        }
    ...
    const fetchChats = async (forceFetch = false) => {
      ...
      try {
        ...
        if (data.data && data.data.documents && isMounted) {
          console.log(
            "✅ Đã fetch được danh sách chat, tổng số:",
          ...
          if (!hasGroupChat && memberId && !isCreatingChat && isMounted) {
            ...
            try {
              console.log(
                "Bắt đầu tạo nhóm chat mặc định cho workspace:",
                workspaceId
              );
              ...
              if (doubleCheckResponse.ok) {
                ...
                if (hasGroup) {
                  console.log(
                    "Phát hiện nhóm chat đã tồn tại trong lần kiểm tra thứ hai, hủy tạo mới"
                  );
                  if (isMounted) {
              ...
              if (!initResponse.ok) {
                throw new Error(
                  `Failed to initialize default chat: ${initResponse.status}`
                );
              }
      ...
      if (data.data) {
        ...
        try {
          const messagesResponse = await fetch(
            `/api/chats/${data.data.$id}/messages`
          );
          if (messagesResponse.ok) {
            const messagesData = await messagesResponse.json();
            if (messagesData.data && messagesData.data.documents) {
              const sortedMessages = [...messagesData.data.documents].sort(
                (a, b) => {
                  const timeA = new Date(a.CreatedAt || a.$createdAt).getTime();
                  const timeB = new Date(b.CreatedAt || b.$createdAt).getTime();
                  return timeA - timeB;
        ...
        {error && error.includes("not a member") ? (
          <div className="h-full flex flex-col items-center justify-center p-8">
            ...
            <h2 className="text-2xl font-semibold mb-4">
              Join Workspace to Chat
            </h2>
            <p className="text-muted-foreground text-center max-w-md mb-6">
              You need to be a member of this workspace to access the chat
              feature.
            </p>

    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorInfo = { message: await response.text().catch(() => "Không thể đọc lỗi") };
        }
        
        // Tạo lỗi với thông tin chi tiết
        const errorMessage = errorInfo.message || `Yêu cầu thất bại với mã lỗi ${response.status}`;
        ...
        if (isServerError && attempts <= retries) {
          console.warn(`Lỗi server (${response.status}) khi gọi ${method} ${path}. Thử lại lần ${attempts}/${retries}`);
          lastError = error;

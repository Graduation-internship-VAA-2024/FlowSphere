...
export const waitForWebSocketReady = async (
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (!currentSocket) {
      console.log(`No WebSocket connection found (attempt ${attempt}/${maxAttempts})`);
      continue;
    }
    ...
    if (currentSocket.readyState === WebSocket.CONNECTING) {
      console.log(`WebSocket still connecting (attempt ${attempt}/${maxAttempts})`);
      continue;
    }
    ...
    if (currentSocket.readyState === WebSocket.CLOSED || currentSocket.readyState === WebSocket.CLOSING) {
      console.log(`WebSocket is closed/closing, attempting to reconnect (attempt ${attempt}/${maxAttempts})`);
  ...
  console.error(`Failed to establish WebSocket connection after ${maxAttempts} attempts`);
  return false;
};
